...
- Email detection recognizes whitespace and `, ; : ( ) [ ] < >` as token boundaries, preserving original UTF-8 byte spans and all unused punctuation/text. The local #15 regression fix covers comma-adjacent and repeated addresses, Unicode prefixes, trailing punctuation, and rejected near misses through core and CLI tests; independent review passed and integration is pending.
