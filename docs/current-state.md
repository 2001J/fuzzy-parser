...
- `parser-core` provides serializable canonical raw-document models, source locations, raw values, warnings, structured parser errors, configurable derived text normalization, and initial record segmentation strategies.
...
- Advanced record segmentation for multiple records per line and header/section boundaries.
...
The next implementation slice should extend segmentation with repeated-identifier splits and header/section boundary handling. The project does not yet detect candidates or assign schema fields.
