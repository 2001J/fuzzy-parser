...
- CLI dispatch still falls through to TXT for unknown extensions, but TXT now rejects them through library validation. Explicit CLI dispatch/argument cleanup remains #6; CSV/XLSX/schema limits remain #17. File validation is neither content detection nor a sandbox or stable snapshot; see its [race limits](file-validation.md#compatibility-and-limits).
