...
- The CLI `parse` command uses shared schema decoding/compilation and the versioned `ParseResponse` pipeline. Not-yet-supported field types (`text`, `person_name`, `datetime`) retain `schema_field_type_unsupported`; unsupported options, constraints and enum definitions now fail explicitly using the existing safe error boundary.
